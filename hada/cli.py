...
@click.option(
    '--bbox-m',
    help='Bounding box in meters on EPSG:3575 (xmin, xmax, ymin, ymax)')
...
@click.option('--from',
              't0',
              type=click.DateTime(),
              help='UTC date-time start (default: -1day)')
@click.option('--to',
              't1',
              type=click.DateTime(),
              help='UTC date-time end (default: now)')
@click.option('--freq',
              default='1H',
              type=str,
              help='Time delta in time span.')
@click.option('-d',
              '--dataset-filter',
              multiple=True,
              help='Only include datasets containing string')
@click.option('-v',
              '--variable-filter',
              multiple=True,
...
def hada(log_level, sources, target_epsg, grid, bbox_deg, bbox_m, dx, dy, t0,
    ...
    if bbox_deg is not None and bbox_m is not None:
        logger.error(
            'Only one of bbox_deg and bbox_m can be specified at the same time.'
        )
        return 1
    ...
    if grid is not None:
        ...
        assert len(
            bbox_m
        ) == 4, "Bounding box should consist of 4 comma-separated floats"
    ...
    for var in sources.scalar_variables:
        ...
        vo = sources.regrid(var, target, time, always_valid)
    ...
    for var, vvar in sources.vector_magnitude_variables.items():
        assert len(
            vvar) == 2, "Vector variables can only contain two components"
        varx = vvar[0]
        vary = vvar[1]
        logger.info(f'Searching for variable {varx},{vary}')

        # Acquire variables on target grid
        vox = sources.regrid(varx, target, time, always_valid)
        voy = sources.regrid(vary, target, time, always_valid)
    ...
    for var, vvar in sources.derived_variables.items():
        ...
        for vn in vvar:
            vo = sources.regrid(vn, target, time, always_valid)
            if vo is not None:
                vos.append(vo)
                continue
        ...
        if len(vos) != len(vvar):
            logger.error(
                f'Could not find all necessary variables for {var}, skipping.')
            continue
        else:
